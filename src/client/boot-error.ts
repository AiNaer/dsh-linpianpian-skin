/**
 * 内核加载失败页（「Failed to load plugins」）的书斋化装饰：不替换内核的
 * 诊断报告，只在页面两侧立人物、报告卡换宣纸册页。此效果依赖本皮肤已成功
 * 执行；皮肤自身或内核尚未加载时保留原生页面。
 *
 * Ported from dsh-deep-whale maid-atelier boot-error.ts (MIT, Small-tailqwq)。
 */
const BOOT_SELECTOR = '[data-dsh-boot]'
const ERROR_ATTRIBUTE = 'data-lpp-boot-error'
...
export function installLppBootError(): () => void {
  const owner = Symbol('lpp-boot-error')
