 ...
 * 宽表格「册页」预览/展开处理。
 *
 * 宿主渲染器把 ≥4 列的表格打上 `md-table-wide` 并撑出消息列。本模块让表格
 * 默认留在原地，只在真正溢出时标记为可展开，并在用户明确点击时打开皮肤自有
 * 的放大克隆（灯箱）。渲染器拥有原表格子树，本模块只在原位装饰其包裹层，
 * 绝不改挂。列数钩子只是发现线索：是否启用看表格与气泡的实时几何。
 *
 * 所有 observer、监听器、注入按钮、浮层与属性写入都归本模块所有，由返回的
 * disposer 释放。
 *
 * Ported from dsh-deep-whale maid-atelier table-card.ts (MIT, Small-tailqwq)，
 * 钩子改为 data-lpp-table-*，样式另见 skin.ts 的册页一节。
 */
import type { Context } from '@deepseek-ai/cordis'

export const LPP_TABLE_SELECTOR = '.md-table-wide'
const SKIN_OWNER = 'linpianpian'
const EXPANDABLE_ATTRIBUTE = 'data-lpp-table-expandable'
const OPEN_ATTRIBUTE = 'data-lpp-table-open'
const CONTROL_ATTRIBUTE = 'data-lpp-table-expand'
const FRAME_ATTRIBUTE = 'data-lpp-table-frame'
const SCROLL_SUPPRESSED_ATTRIBUTE = 'data-lpp-table-scroll-suppressed'
const OVERLAY_ATTRIBUTE = 'data-lpp-table-lightbox'
...
function acquireControl(wrapper: HTMLElement, owner: symbol, activate: () => void): HTMLButtonElement {
  ...
  if (lease === undefined) {
    ...
    button.setAttribute('aria-label', '展卷全览表格')
    button.title = '展卷全览表格'
...
/**
 * 在当前 body 上安装宽表格几何通道。重复调用幂等；一切观察与变更由返回的
 * disposer 拆除。
 */
export function installLppTableCards(_ctx: Context): TableCardRuntime {
  const owner = Symbol('lpp-table-card-activation')
  ...
  const openOverlay = (wrapper: HTMLElement): void => {
    ...
    backdrop.dataset.lppTableBackdrop = ''

    const panel = document.createElement('div')
    panel.dataset.lppTablePanel = ''
    ...
    panel.style.setProperty('--lpp-table-expanded-width', `${targetWidth}px`)

    const close = document.createElement('button')
    close.type = 'button'
    close.dataset.lppTableClose = ''
    close.setAttribute('aria-label', '收起表格')

    const scroller = document.createElement('div')
    scroller.dataset.lppTableExpandedScroller = ''
    ...
    clone.dataset.lppTableExpanded = ''
    ...
    const onClick = (event: MouseEvent): void => {
      ...
      if (target instanceof Element
        && (target.closest('[data-lpp-table-close]') !== null || target.hasAttribute('data-lpp-table-backdrop'))) {
        closeOverlay()
      }
    }
  ...
  try {
    ...
    observer = new MutationObserver((records) => {
      ...
      for (const record of records) {
        if (record.target instanceof HTMLElement) {
          const wrapper = record.target.matches(LPP_TABLE_SELECTOR)
            ? record.target
            : record.target.closest<HTMLElement>(LPP_TABLE_SELECTOR)
        ...
        for (const node of record.addedNodes) {
          ...
          if (node.matches(LPP_TABLE_SELECTOR)) track(node as HTMLElement)
          else if (node.querySelectorAll(LPP_TABLE_SELECTOR).length > 0) {
            node.querySelectorAll<HTMLElement>(LPP_TABLE_SELECTOR).forEach(track)
          }
        }
        for (const node of record.removedNodes) {
          if (!(node instanceof Element)) continue
          if (node.matches(LPP_TABLE_SELECTOR)) removed.add(node as HTMLElement)
          node.querySelectorAll<HTMLElement>(LPP_TABLE_SELECTOR).forEach(wrapper => removed.add(wrapper))
        }
      }
    ...
    document.querySelectorAll<HTMLElement>(LPP_TABLE_SELECTOR).forEach(track)
    return runtime
  } catch (error) {
    runtime.dispose()
    throw error
